...
def booking(request):
    ...
    if user:
        ...
        booknow(userid=user, event_name=title, event_picture=epic, speaker_name=sname, speaker_picture=spic, city=city,
                hotel=hotel, event_date=edate, event_price=price, booking_date=datetime.now().date()).save()
        return HttpResponse("<script>alert('Your event is Booked Successfully'); location.href='/user/event/'</script>")
